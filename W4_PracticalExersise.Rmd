...
```
`key words:`ToothGrowth data, Data Exploration, T Tests, Box Plots, Data Analysis, Statistical Inference.
...
```{r, Plot_data,  fig.width = 5, fig.height =2, fig.asp = .618,echo=FALSE}
plot(ToothGrowth)
```
...
```{r, DOSE_box_plots,   fig.width = 5, fig.height =2, fig.asp = .618, echo=FALSE }
...
Let's plot that: 
```{r, SUPPbox_plots,  fig.width = 5, fig.height =2, fig.asp = .618, echo=FALSE }
...
- The  the exercise can be replicated [hidden code is available here](http://rpubs.com/lindangulopez/709101).
